/** Aligned with escrow_web patterns for admin catalogue screens. */
export const fieldLabel =
  "block text-sm font-medium text-gray-700 mb-2 tracking-tight dark:text-gray-200";

export const fieldInput =
  "w-full rounded-xl border border-gray-300 bg-gray-50 px-4 py-3 text-gray-900 placeholder:text-gray-400 " +
  "focus:ring-2 focus:ring-gambian-blue focus:border-transparent outline-none transition shadow-sm " +
  "dark:border-gray-600 dark:bg-gray-900/80 dark:text-gray-100 dark:placeholder:text-gray-500";

export const cardPanel =
  "rounded-2xl border border-gray-100 bg-white/95 shadow-xl shadow-gray-200/40 backdrop-blur-sm " +
  "dark:border-gray-800 dark:bg-gray-900/95 dark:shadow-black/20";
